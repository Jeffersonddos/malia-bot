...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode, isPrems }) => {
   ...
   // ------- OTHER ------
 if (global.db.data.chats[m.chat].nsfw == false && m.isGroup) throw 'O modo horny está desabilitado neste grupo. Ative!'
...
let api = '?apikey=ea7f3def37ca8dad8578250f'

//> Lolhuman
let resl = 'https://api.lolhuman.xyz/api/random2/'
let apil = '?apikey=ea7f3def37ca8dad8578250f'
...
//---------- TEXT -----------
let next = 'ᴘʀᴏxɪᴍᴏ'
let fot = 'Comando Exlusivo para Usuários Premium!'
let txtprem = '❗ Para ter 100% de aproveito, adquira já o seu vip! 📞'
let p = '🅟 | '
let f = 'Ⓕ | '

let tekk = `\`\`\`➩ Imagem Aleatória: ${args[0] ? args[0].capitalize() : false}\`\`\` `
 let teks = `┊ 📮 Selecione a categoria abaixo!
┊› Também pode digitar ${usedPrefix}nsfw neko
❏──···––`
//---------------------------
...
case 'ahegao':
       ...
       if (!isPrems) return m.reply('Comando Exlusivo para Usuários Premium!')
       }
       try{
         conn.sendMessage(m.chat, { image: { url: cita9 }, caption: `_${tekk}_`}, { mentions: [m.sender] })
...
case 'anal':
        ...
        conn.sendMessage(m.chat, { image: await(await fetch(resl + 'anal' + apil)).buffer(), caption: `_${tekk}_`}, { mentions: [m.sender] })
...
case 'eba':
        ...
        conn.sendMessage(m.chat, { image: await(await fetch(cita16)).buffer(), caption: `_${command}_`}, { mentions: [m.sender] })
           break
...
default:
  ...
  const trimmedMessage = message.trim();
        return await
        conn.reply(m.chat, `_${trimmedMessage}_`, m, { mentions: [m.sender] });

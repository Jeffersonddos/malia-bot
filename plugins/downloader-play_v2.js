...
if (!text) throw `[ ❗ ] COLOQUE O NOME DA MUSICA QUE DESEJA NA FRENTE DO COMANDO*`
try {
if (command == 'play.1') {
conn.reply(m.chat, `*_⏰ Estou processando, aguarde..._*`, m) 
try {  
...
if (command == 'play.2') {
conn.reply(m.chat, `*_⏰ Estou processando o vídeo, aguarde..._*`, m)    
try {   
let mediaa = await ytPlayVid(text)
await conn.sendMessage(m.chat, { video: { url: mediaa.result }, fileName: `error.mp4`, caption: `_Malia - Robot_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4' }, { quoted: m })   
} catch {
let res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`)    
let json = await res.json()
await conn.sendFile(m.chat, json.result.video, 'error.mp4', `_Malia - Robot_`, m)}}
} catch (e) {

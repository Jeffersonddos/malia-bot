...
if (!effect) m.reply('*[ ❗ ] COMO USAR O COMANDO?*\n—◉ _#logo (efecto) (texto)_\n*EXEMPLO:*\n—◉ _#logo 3d-deep-sea-metal Mystic_\n\n*[❗] Quando usar com texto:*\n—◉ _#logo (efeito) (texto1|texto2)_\n*EXEMPLO:*\n—◉ _#logo Wolf-Logo-Galaxy Malia|Bot_\n\n*<𝑳𝑰𝑺𝑻𝑨 𝑫𝑬 𝑬𝑭𝑬𝑰𝑻𝑶𝑺/>*\n\n° ඬ⃟📝 #logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 #logo '))
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `O efeito digitado: "${effect || 'nada foi digitado'}" não está na linha de efeito.`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `O efeito digitado: "${effect || 'nada foi digitado'}" não está na linha de efeito.*` : `*O uso correto é ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*Aqui está sua imagem!!*\n*Efeito: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['nulis']

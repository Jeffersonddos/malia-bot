...
let handler = async (m, { conn }) => {
  try {
    const response = await fetch(pickRandom(hnt_gifs));
    const media = await response.buffer();
    const stiker = await sticker(media, false, '↓🧁⃟ Mαℓια Roɓοτ\n⤷fir3.net/malia-md\n\n↓🌈⃟ Oᴘɪɴɪᴀ̃σ ∂α ʙσᴛ\n⤷', `↓🍓⃟ Cʀιαᴅᴀ ᴘᴏʀ\n⤷@david.sousz\n\n↓🧃⃟ Fᴇιτα ᴘοʀ\n⤷${m.pushName}`);
    const out = await webp2mp4(media);
    await conn.sendFile(m.chat, out, 'video.gif', 'Niih', m);
  } catch (error) {
    console.error(error);
    conn.sendMessage(m.chat, 'Ocorreu um erro ao processar o GIF.', m);
  }
};

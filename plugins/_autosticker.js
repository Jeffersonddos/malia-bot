...
if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return await this.sendButton(m.chat, '*[ ❗ ] O VÍDEO NÃO PODE CONTER MAIS QUE 7s*', wm, [['DESATIVAR AUTOFIGU', '/disable autosticker']], m)
let img = await q.download()
if (!img) return
...
//await this.sendFile(m.chat, stiker, 'error.jpg', null, m, false, { contextInfo: { mentionedJid: [m.sender], externalAdReply: { showAdAttribution: true, mediaType: 'VIDEO', mediaUrl: null, title: '𝙲𝚄𝙴𝙽𝚃𝙰𝚂 𝙾𝙵𝙸𝙲𝙸𝙰𝙻𝙴𝚂', body: '𝙱𝚈 @𝐵𝑟𝑢𝑛𝑜𝑆𝑜𝑏𝑟𝑖𝑛𝑜', thumbnail: img, sourceUrl: `https://www.instagram.com/david.sousz`}}})  
}}
return !0
}
